# 19 — Data workflows: catalog reload and chart seed, kept separate
...
- [ ] Both use the lower-privilege Neon role (issue 14), not owner
- [ ] Both take a `pg_dump` first (issue 20) and upload it as a run artifact
- [ ] `seed-charts` fails the run on unmatched titles (issue 10)
