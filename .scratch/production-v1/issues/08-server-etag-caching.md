# 08 — Server: `ETag` / `Cache-Control` / `304` on catalog and manifest
...
strategy — the IndexedDB blob cache (issue 22 / v1.1) layers on top of it, not
instead of it.

**Blocked by:** 07 (interacts with compression — ETag must be stable across
