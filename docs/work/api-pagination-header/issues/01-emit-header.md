...
- [ ] `search_sheets` sets `X-Total-Count` to the same value as the body `total`
- [ ] `CorsLayer` gains `.expose_headers([HeaderName::from_static("x-total-count")])`
- [ ] A test asserts the header is present and equals the body field — the
      invariant that makes step 03 safe to take later
- [ ] A test through the assembled `Router` (`tower::ServiceExt::oneshot`, as
...
- [ ] `api-contract.md` §1.2 notes both are sent during the migration
