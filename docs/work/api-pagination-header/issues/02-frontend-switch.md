...
- [x] Reads `response.headers.get("X-Total-Count")`, parsed to a number
- [x] A missing or unparseable header falls back rather than rendering `NaN`
...
- [x] `pnpm build` clean (`vue-tsc --noEmit` + `vite build`)
- [x] Verified against a running server, not only by type-check — the CORS
      behaviour this depends on does not exist at compile time

## Verified

Against a locally running server on a throwaway database, cross-origin
(`Origin: http://localhost:5173`, server on `:3999`):
...
`?pageSize=1` returned one sheet with `x-total-count: 2`, confirming the header
carries the pre-pagination total rather than the page length. The body still
carried `"total":2` — the expand step, removed by ticket 03.
