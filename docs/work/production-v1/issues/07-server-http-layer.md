# 07 — Server: compression, CORS allowlist, configurable bind
...
3. **Bind address** comes from config (issue 02) so Fly's `PORT` is honoured.
...
- [ ] Comment at the CORS layer stating plainly that CORS is an egress control
      here, not a security control — the real cap is issue 09
