# 18 — Scheduled upstream refresh, delivered as a PR

**What to build:** A cron workflow that fetches upstream `data.json`, and if it
differs from the vendored snapshot (issue 17), opens a pull request with the
...
- [ ] Merging the PR does **not** auto-deploy — applying it is issue 19
